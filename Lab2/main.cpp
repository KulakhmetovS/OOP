...
Student tmpObject;  //Объект для перегрузки операторов
...
int main()
{
    ...
    int size = 0, i, choice = 0, year = 0, new_size;

    cout << "Введите число студентов в списке: ";
    getline(cin, s_size);
    size = stoi(s_size);    //Преобразование строки в число
    new_size = size;
    Student *stud_array = new Student[100];    //Массив объектов класса Student
    ...
    while(1)
    {
        ...
        cout << "4 - Вся информация о студентах\n";
        cout << "Номер действия: ";
        ...
        {
            //Создание объекта для вывода информации о студентах
            Observer observer;

            for(i = 0; i < size; i++)
                observer.GetInfo(stud_array[i]);    //Вывод информации о студентах ввиде строки
        }
    }
    ...
    while(1)
    {
        ...
        cout << "2 - Удалить студента из списка\n";
        cout << "3 - Просмотреть список студентов\n";
        cout << "Номер действия: ";
        getline(cin, s_choice);
        choice = stoi(s_choice);
        ...
        {
            SetData(stud_array[new_size], 0);

            //Добавление нового студента в список через перегруженный оператор "+"
            list + stud_array[new_size];

            new_size++; //Увеличение области элементов на 1
        }
        else if (choice == 2)
        {
            string surname, name;
            ...
            //Проходим все существующие элементы и сравниваем их поля с искомыми
            for(i = 0; i < new_size; i++)
                if((stud_array[i].getSurname() == surname) && (stud_array[i].getName() == name))
                    list - stud_array[i];   //Удаление студента из списка через перегруженный оператор "-"
        }
        else if(choice == 3) {list.print();}
    }
...
void SetData(Student& student, int index)
{
    ...
    tmpObject.setName(surname, name, patronymic); //Ввод фамилии, имени и отчества

    cout << "Введите дату рождения (дд.мм.гггг): ";
    getline(cin, date);

    tmpObject.setDateOfBirth(date);   //Ввод даты рождения
    ...
    tmpObject.setContacts(address, phone_number, faculty);

    cout << "Введите курс: ";
    getline(cin, s_course);
    course = stoi(s_course);

    tmpObject.setCourse(course);  //Ввод курса

    cout << endl;

    student = tmpObject;    //Перегрузка оператора "="
}
